...
# Project 2 - Building an HTTP Server from scratch
...
The Zig Standard Library already has an HTTP Server
...
First of all, what is an HTTP Server?
An HTTP server, as any other type of server, is essentially
...
So, web servers (which is just a fancy name for
...
Imagine an HTTP Server as if it were the receptionist of a large hotel. In a hotel,
you have a reception, and inside that reception there is a receptionist
waiting for customers to arrive. An HTTP Server is essentially a receptionist
...
Then, the receptionist searches for an available apartment. If there is an available apartment
...
That is, in a nutshell, what an HTTP Server does. It waits for clients to connect to the
...
This HTTP Request is an HTTP message that contains what
...
There are different "types of request" that a client can send to an HTTP Server,
but the most basic type is when a client asks the HTTP Server to serve (i.e. to send) some specific 
web page (which is an HTML file) to him. When you type `google.com` in your web browser, you are 
essentially sending an HTTP Request to Google's HTTP servers. This request is asking these servers 
to send the Google webpage to you.

Nonetheless, when the server receives this first message-the *HTTP Request*-it
analyzes this request to understand: Who the client is? What does the client want the server to do?
Has the client provided all the necessary information to perform the requested action?

Once the server understands what the client wants, it simply performs the requested action.
To complete the whole process, the server sends an HTTP message back to the client, informing 
him whether the action performed was successful. Finally, the server ends (or closes) 
the connection with the client.

This last HTTP message sent from the server to the client is called the *HTTP Response*.
Since the server is responding to the action that was requested by the client,
the main objective of this response message is to let the client know whether the
action requested was successful before the server closes the connection.


## How an HTTP server is normally implemented? {#sec-http-how-impl}
...
such an HTTP server in C.


In essence, we normally implement an HTTP server in C by using WebSocket technology,
...
1. When a connection arrives, we accept this connection, and we exchange the HTTP messages (HTTP Request and HTTP Response).
...
in your hands. However, this channel is not currently available, nor is it accessible,
as it does not have a known address where you can find it.

That is what the "bind" operation does. It binds a name (or more specifically, an address) to
this socket object, or this channel of communication, so that it becomes available and accessible 
through this address. While the "listen" operation makes the socket object 
